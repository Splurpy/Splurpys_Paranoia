package net.splurpy.paranoiamod.screen;
...
public class MortarAndPestleMenu extends AbstractContainerMenu {
    ...
    public MortarAndPestleMenu(int id, Inventory inv, BlockEntity entity, ContainerData data) {
        ...
        this.blockEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(handler -> {
            this.addSlot(new SlotItemHandler(handler, 0, 26, 33));
    ...
    // Number of slots in BlockEntityMenu
    ...
    private void addPlayerInventory(Inventory playerInventory) {
        for (int i = 0; i < 3; i++) {
            for (int l = 0; l < 9; l++) { //   INV                #                      ?                 Init +      ?
                this.addSlot(new Slot(playerInventory, l + i * 9 + 9, 8 + l * 18, 84 + i * 18)); // Changed final arg to * 16 (from * 18) -- Middle row is fine, but top and bottom are offset by two vertically
            } //                                                                                           86
        }
    }

    private void addPlayerHotbar(Inventory playerInventory) {
        for (int i = 0; i < 9; i++) { //    INV    #          ?              Height
            this.addSlot(new Slot(playerInventory, i, 8 + i * 18, 142)); // Changed final arg from 144 to 142 -- THIS WORKED
        }
    }
}
